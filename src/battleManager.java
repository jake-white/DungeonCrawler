...
public class battleManager {
	...
	public static int hp = 100, ehp, mana = 20, damage, monsterType, maxhp = 100, maxmana = 20;
	public static String output;
	public static String monster[] = new String[5];
	public static int e_basedamage[] = new int[5];
	public static double e_def[] = new double[5];
	public static int basedamage, fire_basedamage, def, e_damage;
	...
	public static void battleInit() {
		...
		monsterType = generator.nextInt(3) + 1;
		switch (monsterType) {
		case 1:
			ehp = 20;
			break;
	...
	public static void battleMechanics() {
		if (!inputManager.spaceHandler(KeyEvent.VK_ENTER) && hp > 0 && ehp > 0) {
			...
			if (selected == 1) {
				...
				if (mapManager.key_get)
					...
					damage = (int) Math.round((basedamage / e_def[monsterType])
							+ generator.nextInt(3));
				e_damage = Math.round((e_basedamage[monsterType + 1] / def)
						+ generator.nextInt(5));
				ehp -= damage;
				hp -= e_damage;
		...
		if (ehp <= 0 && System.currentTimeMillis() > timer + 1000) {
			encounter = false;
